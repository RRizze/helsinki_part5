  ...
  return (
    <div className='blog' style={blogStyle} >
      <div>
        <p>{blog.title}</p>
        <p>{blog.author}</p>
        <button id='blog-show' className='btn-toggle' onClick={toggleVisibility}>show</button>
      </div>

      <div className='blog-url-likes' style={infoStyle}>
        <a className='blog-url' href='#'>{blog.url}</a>
        <div className='blog-likes'>
          <span>likes:</span> {blog.likes}
          <button id='add-like' onClick={(e) => handleAddLike({ e, blog })}>like</button>
        </div>
